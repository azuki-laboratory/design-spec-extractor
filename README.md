...
이미지 생성 AI로 고품질 원화를 만들어 아이콘으로 쓸 수 있다:

1. ChatGPT/Gemini/Midjourney 등에서 아래 프롬프트로 생성
2. 결과를 `icons/azuki-art.png`로 저장 (정사각, 512px 이상 권장)
3. `npm run icons` → 16/48/128 자동 생성 → `chrome://extensions` 리로드

원화가 없으면 `icons/azuki.svg` 벡터로 폴백.

**생성 프롬프트 예시:**

> Retro 1970s-80s shoujo manga style portrait of a young girl with rabbit ears
> on a headband, blunt bangs bob haircut, calm serious expression.
> High-contrast monochrome manga ink style: black fills with white highlights on
> white background. Bold flat shapes, glossy hair shine bands, clean vector-like
> feel, thin frame border. Square composition, centered head-and-shoulders,
> simple background, no text. App icon, readable at small sizes.
