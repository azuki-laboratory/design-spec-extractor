...
$('ai-generate').addEventListener('click', async () => {
  ...
  try {
    ...
    // 저온도/저topK: 소형 모델의 구조·프롬프트 준수 안정화 (지원 범위 내로 클램프)
    const params = await LanguageModel.params().catch(() => null);
    const sampling = params ? {
      temperature: Math.min(0.4, params.maxTemperature ?? 1),
      topK: Math.max(1, Math.min(3, params.maxTopK ?? 8)),
    } : {};
    const session = await LanguageModel.create({
      // few-shot 예시 1쌍 포함 — 형식·요청 반영도를 크게 올린다
      initialPrompts: [
        { role: 'system', content: DesignGenerator.customKitSystemPrompt(opts.lang, opts.aiCustom) },
        ...DesignGenerator.customKitFewShot(opts.lang),
      ],
      ...sampling,
      monitor(m) {
    ...
    phase = t().aiWorking;
    // 브랜드 컨텍스트 주입 — 결과 카피가 분석한 디자인·사이트와 어울리게
    const dna = DesignGenerator.computeDNA(lastData, opts.lang).slice(0, 5).join(', ');
    const brandName = (lastData.meta.title || '').split(/[—|·:-]/)[0].trim().slice(0, 40);
    const input = `Design context: ${dna}.${brandName ? ` Brand: ${brandName}.` : ''}\nUser request: ${promptText}`;
    // 스키마 강제(responseConstraint) 우선, 미지원 브라우저는 자유 텍스트로 폴백.
    // 스트리밍 수신 — 응답이 흘러오는 동안 수신 글자 수로 "살아있음"을 보여준다
    const runPrompt = async (promptOpts) => {
      let out = '';
      if (session.promptStreaming) {
        const stream = session.promptStreaming(input, promptOpts);
        for await (const chunk of stream) {
          out += chunk;
          phase = t().aiReceiving(out.length);
        }
        return out;
      }
      return session.prompt(input, promptOpts);
    };
    let raw;
    try {
      raw = await runPrompt({ responseConstraint: DesignGenerator.KIT_STRUCTURE_SCHEMA });
    } catch (constraintErr) {
      raw = await runPrompt(undefined);
    }
    session.destroy();
