...
<main class="section"><div class="container stack">
  ...
  <div class="docs">
    ...
    <article class="stack">
      ...
      <p>${T('Unzip the kit and open index.html. Edit assets/css/tokens.css to adjust the design — every page follows.', '키트 압축을 풀고 index.html을 여세요. assets/css/tokens.css를 수정하면 모든 페이지에 반영됩니다.')}</p>
...
// 파비콘: primary 색 원 + 브랜드 이니셜 (자기완결 SVG)
function faviconSvg(brand, tokens) {
  const primary = cssSafe(tokenHex(tokens, 'primary', tokenHex(tokens, 'link', '#3b82f6')));
  const on = cssSafe(tokenHex(tokens, 'on-primary', '#ffffff'));
  const initial = htmlEsc(String(brand || 'A').trim().charAt(0).toUpperCase());
  return `<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64">
<circle cx="32" cy="32" r="30" fill="${primary}"/>
<text x="32" y="43" font-family="-apple-system, sans-serif" font-size="32" font-weight="700" fill="${on}" text-anchor="middle">${initial}</text>
</svg>
`;
}

// 404: 배포 시 Vercel/Netlify가 자동 인식
function notFoundHtml(data, brand, pages) {
  const body = `${navHtml(brand, pages, null)}
<main class="section"><div class="container center stack" style="gap:16px;max-width:520px;margin:0 auto;padding:80px 0">
  <span class="eyebrow">404</span>
  <h1>${T('Page not found', '페이지를 찾을 수 없어요')}</h1>
  <p class="muted">${T("The page you're looking for was moved or never existed.", '찾으시는 페이지가 이동했거나 없는 주소예요.')}</p>
  <div class="row" style="justify-content:center"><a class="btn-primary" href="index.html">${T('Back to home', '홈으로 돌아가기')}</a></div>
</div></main>
${footerHtml(brand, pages)}`;
  return page(`${brand} — 404`, body);
}

// 사이트맵: 배포 도메인으로 교체해 사용
function sitemapXml(pages) {
  const urls = pages.map((p) => `  <url><loc>https://example.com/${PAGE_META[p].file}</loc></url>`).join('\n');
  return `<?xml version="1.0" encoding="UTF-8"?>
<!-- ${T('Replace example.com with your deployed domain', 'example.com을 배포 도메인으로 교체하세요')} -->
...
| \`404.html\` | Not-found page (auto-served by Vercel/Netlify) |
| \`assets/css/tokens.css\` | **Design tokens — edit this file first** |
| \`assets/css/kit.css\` | Component classes built on the tokens |
| \`assets/favicon.svg\` | Favicon in your primary color |
...
| \`robots.txt\` / \`sitemap.xml\` | SEO crawler rules + sitemap |
| \`.gitignore\` | Ready for git init |
| \`vercel.json\` / \`netlify.toml\` | One-click deploy configs |

## Quick start

1. Unzip, open \`index.html\` — no build step.
2. Edit \`assets/css/tokens.css\`; every page follows.
...
| \`404.html\` | 404 페이지 (Vercel/Netlify 자동 인식) |
| \`assets/css/tokens.css\` | **디자인 토큰 — 가장 먼저 수정할 파일** |
| \`assets/css/kit.css\` | 토큰 위에 만든 컴포넌트 클래스 |
| \`assets/favicon.svg\` | primary 색 파비콘 |
| \`snippets/analytics.html\` | GA4 + PostHog 스니펫 (\`<head>\`에 붙여넣기) |
| \`i18n/strings.json\` | 다국어 스타터 문자열 (en/ko) |
| \`llms.txt\` | LLMO — AI 검색엔진용 사이트 요약 |
| \`robots.txt\` / \`sitemap.xml\` | SEO 크롤러 규칙 + 사이트맵 |
| \`.gitignore\` | git init 준비 완료 |
| \`vercel.json\` / \`netlify.toml\` | 원클릭 배포 설정 |

## 빠른 시작

1. 압축 해제 후 \`index.html\` 열기 — 빌드 불필요.
2. \`assets/css/tokens.css\`만 고치면 모든 페이지에 반영.
...
export function exportKit(data, lang, kitOpts) {
  ...
  files.push({ name: '404.html', content: notFoundHtml(data, brand, pages) });
  files.push({ name: 'assets/css/tokens.css', content: exportTokens(data, lang).css });
  files.push({ name: 'assets/css/kit.css', content: buildKitCss(data, tokens) });
  files.push({ name: 'assets/favicon.svg', content: faviconSvg(brand, tokens) });
  ...
  files.push({ name: 'sitemap.xml', content: sitemapXml(pages) });
  files.push({ name: '.gitignore', content: gitignoreTxt() });
...
// Prompt API responseConstraint용 JSON 스키마 — 소형 모델의 스키마 이탈을 원천 차단
export const KIT_STRUCTURE_SCHEMA = {
  type: 'object',
  ...
  properties: {
    ...
    sections: {
      ...
      items: {
        ...
        properties: {
          type: { enum: ['hero', 'logos', 'features', 'stats', 'testimonials', 'pricing', 'faq', 'cta'] },
          eyebrow: { type: 'string' }, title: { type: 'string' }, body: { type: 'string' },
          cta: { type: 'string' }, cta2: { type: 'string' }, note: { type: 'string' },
          items: {
            type: 'array', maxItems: 8,
            items: {
              type: 'object',
              properties: {
                icon: { type: 'string' }, badge: { type: 'string' }, title: { type: 'string' },
                body: { type: 'string' }, value: { type: 'string' }, name: { type: 'string' },
                role: { type: 'string' }, price: { type: 'string' }, cta: { type: 'string' },
                featured: { type: 'boolean' }, features: { type: 'array', items: { type: 'string' } },
              },
            },
          },
        },
...
// Few-shot 예시 (user → assistant 1쌍) — 소형 모델의 형식·프롬프트 준수를 크게 끌어올린다
export function customKitFewShot(lang) {
  const ko = lang === 'ko';
  const userMsg = ko ? '동네 필라테스 스튜디오 소개 페이지 — 수업 종류와 후기, 체험 신청 포함'
    : 'Intro page for a local pilates studio — class types, reviews and a trial sign-up';
  const example = {
    title: ko ? '코어핏 필라테스' : 'CoreFit Pilates',
    brand: ko ? '코어핏' : 'CoreFit',
    sections: [
      { type: 'hero', eyebrow: ko ? '신규 오픈' : 'Now open', title: ko ? '몸의 중심을 다시 세우다' : 'Rebuild your core strength',
        body: ko ? '소규모 그룹 레슨으로 자세와 코어를 함께 잡아드립니다. 초보자도 첫 수업부터 편안하게.' : 'Small-group sessions that fix posture and build core strength — beginners welcome from day one.',
        cta: ko ? '체험 수업 신청' : 'Book a trial class', note: ko ? '첫 체험 무료 · 예약 후 취소 가능' : 'First trial free · cancel anytime' },
      { type: 'features', title: ko ? '수업 종류' : 'Classes', items: [
        { icon: '🧘', title: ko ? '기구 필라테스' : 'Reformer', body: ko ? '리포머 기구로 정확한 자세 교정' : 'Precise posture work on the reformer' },
        { icon: '🤸', title: ko ? '매트 클래스' : 'Mat class', body: ko ? '소도구를 활용한 전신 밸런스' : 'Full-body balance with props' },
        { icon: '👥', title: ko ? '1:1 퍼스널' : '1:1 personal', body: ko ? '개인 목표에 맞춘 맞춤 레슨' : 'Sessions tailored to your goals' },
      ] },
      { type: 'testimonials', title: ko ? '수강생 후기' : 'What members say', items: [
        { name: ko ? '김지현' : 'Jihyun Kim', role: ko ? '6개월 수강' : 'Member, 6 months', body: ko ? '허리 통증이 정말 줄었어요. 선생님이 자세 하나하나 봐주세요.' : 'My back pain is nearly gone — the coaching is incredibly detailed.' },
      ] },
      { type: 'cta', title: ko ? '이번 주 체험 수업, 자리 얼마 안 남았어요' : 'Trial spots are limited this week',
        cta: ko ? '지금 신청하기' : 'Reserve now', note: ko ? '전화 없이 온라인으로 30초 만에' : 'Book online in 30 seconds' },
    ],
  };
  return [
...
// customInstruction: 사용자가 설정 페이지에서 지정한 추가 지시 (톤·업종 규칙 등)
export function customKitSystemPrompt(lang, customInstruction) {
  const language = lang === 'ko' ? 'Korean' : 'English';
  const extra = customInstruction ? `\nAdditional user rules (follow strictly): ${String(customInstruction).slice(0, 500)}` : '';
  return `You are a landing page architect. Given a user's description of a page, respond with ONLY a JSON object (no markdown fences, no commentary) describing the page structure. All user-facing text must be written in ${language}. Every section's copy must be about the user's described business — reuse their words (business type, offerings, audience) in titles and bodies.${extra}
Schema:
