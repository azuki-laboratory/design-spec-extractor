...
const DesignGenerator = (() => {
  ...
  function generate(data, lang) {
    ...
    const tr = (data.depth.transitions || []).map((t) => t.duration).join(', ');
  ...
  return { generate, exportTokens, merge, exportPreview, exportTailwind, computeDNA, computeLint, mascotComment, exportAgentPrompt };
})();
